...
def convert_df_to_dataset(df_train: pd.DataFrame, 
    ...
    df_valid_dataset = Dataset.from_pandas(df_valid)
    
    # TODO: Combine them into a single DatasetDict
    dataset_dict = DatasetDict({
        'train': df_train_dataset,
        'validation': df_valid_dataset
    })
    return dataset_dict
...
def predict_sentiment(tokenizer, model,
    ...
    tokenized_text = tokenizer(text, return_tensors="pt", truncation=True, padding="max_length", max_length=MAX_LENGTH)
...
def get_accuracy(list_gt: np.array, 
    ...
    mask = (list_gt == list_pred)
    return mask.sum()/len(mask)

def plot_result_1(plot_name:str, val_acc_:list, test_acc_:list, epoch_:list, data_sizes_:list, learning_rates_:list) -> None:
    """
    ...
    plt.plot(epoch_, val_acc_, label='Validation Accuracy', marker='o')
    plt.plot(epoch_, test_acc_, label='Test Accuracy', marker='o')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
...
def plot_result_2(plot_name:str, val_acc_:list, data_sizes_:list) -> None:
    """
    Input: Decide on the input that will be relevant to completing the task
    Task: Using a library like matplotlib, you must create two plots:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(data_sizes_, val_acc_, label='Validation Accuracy', marker='o')
    plt.xlabel('Training Data Size')
    plt.ylabel('Accuracy')
    plt.title('Validation Accuracy per Training Data Size')
    plt.xticks(data_sizes_)
    plt.legend()
    plt.tight_layout()
    plt.savefig(plot_name)

def plot_result_3(plot_name:str, val_acc_:list, test_acc_:list, learning_rates_:list) -> None:
    """
    Input: Decide on the input that will be relevant to completing the task
    Task: Using a library like matplotlib, you must create two plots:
    ...
    """
    plt.figure(figsize=(10, 6))
    plt.plot(learning_rates_, val_acc_, label='Validation Accuracy', marker='o')
    plt.plot(learning_rates_, test_acc_, label='Test Accuracy', marker='o')
    plt.xlabel('Learning Rate')
    plt.ylabel('Accuracy')
    plt.title('Validation and Test Accuracy per Learning Rate')
    plt.xscale('log')
    plt.xticks(learning_rates_)
    plt.legend()
    plt.tight_layout()
    plt.savefig(plot_name)
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        if args.best_model_name:
            ...
            plot_result_1(args.plot_name, list_valid_score, list_test_score, list_model_names)
